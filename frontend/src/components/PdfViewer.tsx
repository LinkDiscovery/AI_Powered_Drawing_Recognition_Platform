...
export default function PdfViewer({ file, onSaveSelection, initialSelection, initialBBoxes, activeTool, onToolChange }: PdfViewerProps) {
  ...
  const fitWidth = async (force = false) => {
    ...
    try {
      ...
      if (container && w > 0) {
        // Subtract standard scrollbar width (~17px) + padding (40px) to be safe
        const newScale = (container.clientWidth - 50) / w;
        const roundedScale = Math.floor(Math.min(Math.max(newScale, 0.1), 4.0) * 100) / 100;

        setScale(prev => {
          if (Math.abs(prev - roundedScale) < 0.02) return prev;
          return roundedScale;
        });
      }
    } catch { }
